...
- **Design tokens.** `tailwind.config.js` + `src/theme/index.ts` carry the Rabisko design tokens (source of truth: `design/src/colors_and_type.css` and `design/DESIGN.md`). Border radii: use the `rounded-rd-xs … rounded-rd-pill` classes (anchor = the Cadastro RoleSwitch — 12px outer / 8px inner = `rd-md`/`rd-xs`) — never hardcode `rounded-[24px]`/`rounded-full`/etc. The `rd-` prefix avoids collision with Tailwind's built-in `rounded-r-*` directional modifier (right-only corners). Colors: use semantic tokens (`bg-surface`, `text-ink`, `bg-plum`, `text-fg-2`, …); `plum` (`#602C66`) is the only selection/activation color. The `primary`/`card`/`text`/`muted` color keys and `theme.borderRadius` are deprecated — `design/IMPLEMENTATION-CHECKLIST.md` tracks the migration and the broader design-alignment work (the current screens predate the design system).
